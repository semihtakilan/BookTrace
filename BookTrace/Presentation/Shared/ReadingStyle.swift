...
struct BookIdentityHeader: View {
    let book: BookReference

    var body: some View {
        VStack(spacing: 22) {
            RemoteBookCover(url: book.coverURL, width: 142, height: 213, contentMode: .fit,
            ...
            VStack(spacing: 8) {
                ...
                HStack(spacing: 12) {
                    if let count = book.pageCount { Text("\(count) pages") }
                    if book.pageCount != nil, book.publicationYear != nil {
                        Text(verbatim: "·")
                    }
                    if let year = book.publicationYear { Text(year) }
                }
                .font(.caption)
                .foregroundStyle(ReadingStyle.secondary)
                .padding(.top, 4)
